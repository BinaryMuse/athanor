...
must_haves:
  ...
  artifacts:
    - path: "apps/athanor/lib/athanor/experiments.ex"
      provides: "get_result!/1 function for fetching individual results"
      contains: "def get_result!"
...
<task type="auto">
  ...
  <files>apps/athanor/lib/athanor/experiments.ex, apps/athanor_web/lib/athanor_web/live/experiments/run_live/show.ex</files>
  <action>
First, add `get_result!/1` to the Experiments context (apps/athanor/lib/athanor/experiments.ex):

In the `# --- Results ---` section, add after `get_results_by_key/2`:
...
This follows the existing pattern used for `get_instance!/1` and `get_run!/1`.

Then, modify show.ex to track hydration state per result:
...
The stream_insert with same ID replaces the existing item, which will trigger the component to re-render with the hydrated content.
  ...
  <verify>
`mix compile` succeeds with no warnings in experiments.ex and show.ex. The `get_result!/1` function exists in Experiments context. The `handle_event("hydrate_result", ...)` function exists in show.ex.
  </verify>
  <done>
`get_result!/1` added to Experiments context. Results stream items have `hydrated` field. `hydrate_result` event handler updates the stream item to `hydrated: true`.
  </done>
</task>
