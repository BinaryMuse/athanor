...
must_haves:
  truths:
    ...
    - "Indeterminate spinner shows when running without progress data"
    - "Run completion/failure toast notification visible from any page (global PubSub)"
  artifacts:
    ...
    - path: "apps/athanor_web/lib/athanor_web/live/experiments/instance_live/show.ex"
      provides: "PubSub subscription for global run completion toasts"
      contains: "experiments:runs:active"
  key_links:
    ...
    - from: "apps/athanor_web/lib/athanor_web/live/experiments/instance_live/show.ex"
      to: "Phoenix.PubSub"
      via: "subscribes to experiments:runs:active for global completion toasts"
      pattern: "subscribe.*experiments:runs:active"
---
         ...
         <header class="sticky top-0 z-10 bg-base-100 border-b border-base-300 shadow-sm">
           <div class="px-4 sm:px-6 lg:px-8 py-3">
             ...
             <div class="flex items-center gap-4 flex-wrap">
               ...
               <ProgressBar :if={running and @progress exists} /> or <progress class="progress w-24" :if={running and no @progress} /> (indeterminate)
...
<task type="auto">
  <name>Task 4: Add global PubSub subscription for run completion toasts</name>
  <files>apps/athanor_web/lib/athanor_web/live/experiments/instance_live/show.ex</files>
  <action>
    Add global run completion toast notifications to InstanceLive.Show (and any other page where user may navigate during a run):

    1. **In mount, add subscription to global runs topic:**
       ```elixir
    ...
    2. **Add handle_info for :run_completed:**
       ```elixir
    ...
    Note: The flash component in CoreComponents already renders as a daisyUI toast (`toast toast-top toast-end`), so the notification will appear correctly. The run page (RunLive.Show) does NOT need this subscription because it shows status directly in the sticky header — skip the flash there to avoid redundancy.

    Reference: Research Pattern 7 for global toast implementation
  </action>
  <verify>
    - grep for `experiments:runs:active` in instance_live/show.ex
    - grep for `:run_completed` in instance_live/show.ex
    - grep for `put_flash` in instance_live/show.ex
  </verify>
  <done>
    - InstanceLive.Show subscribes to experiments:runs:active PubSub topic
    - Run completion/failure/cancellation triggers toast notification via put_flash
    - Toast appears on any page where user is viewing (not just run page)
  </done>
</task>
...
9. **Global run completion toast subscription:**
   ```bash
   grep -n "experiments:runs:active" lib/athanor_web/live/experiments/instance_live/show.ex
   grep -n ":run_completed" lib/athanor_web/live/experiments/instance_live/show.ex
   ```
</verification>
...
9. Running without progress data shows indeterminate spinner (daisyUI progress element without value)
10. Run completion/failure toast appears on any page via global PubSub
11. Application compiles without warnings
</success_criteria>
