...
5. Implement `config_scalar_field/1` with pattern matching on field_def.type:
...
6. Field layout pattern (stacked labels per user decision):
   ```heex
...
8. Replace `form-control` class with `fieldset mb-4` (per research - form-control deleted in daisyUI 5)

9. Use `input w-full`, `textarea w-full`, `select w-full`, `checkbox` classes
...
Module exports config_form/1 function.
  ...
  <done>
ConfigFormComponent renders all scalar field types (string, integer, boolean, enum) with proper styling: stacked labels, required asterisks, help text, format-aware inputs, error state classes. Ungrouped fields are wrapped in a default bg-base-200 card per user decision.
  </done>
</task>
...
5. Model pairs list field shows placeholder (list handling comes in Plan 03)
